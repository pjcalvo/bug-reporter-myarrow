var imageURL, os, browser, webURL, environment, environmentMatch, urls;
...
function testURLMatches(url) {
    ...
    for (i=matches.length-1; i>=0; i--) {
        ...
        if (r.test(url)) {
            ...
            environmentMatch = config.testing_environments[i].url_format;
            hide('#verifying');
            show('#loading');
            return true;
        }
...
function openPage() {
    ...
    function onwriteend() {
        ...
        chrome.history.search({text:environmentMatch, startTime:600000, maxResults:10}, function(results) {
            console.log('Resultados del history: ');
            var urlsTemp = '';
            results.forEach(function(historyResult) {
                console.log(historyResult.url);
                urlsTemp = urlsTemp + '* ' + historyResult.url + '\n';
              });
              urls = urlsTemp;
              $('#urls').append(urls);
        });
...
chrome.tabs.getSelected(null, function(tab) {
    ...
    if (testURLMatches(tab.url)) {
        var loaded = false;
            ...
            chrome.tabs.executeScript(tab.id, {file: '/assets/js/page-capture.js'}, function() {
            loaded = true;
...
function openJira(){
    ...
    var environmentData = {"imageURL": imageURL,
        ...
        "jiraUrl" : config.bug_tracking_tool.url,
...
document.getElementById("submit").addEventListener("click", openJira);
